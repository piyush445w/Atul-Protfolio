...
def _save_software(data):
    ...
    logger.info("Software saved successfully")
...
def admin_dashboard():
    ...
    content = f"""
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                <div class="stat-value">{len(settings.get('awards', []))}</div>
        ...
        <div class="quick-actions">
            ...
            <div style="display: flex; gap: 12px; flex-wrap: wrap;">
                ...
                <a href="/admin/settings#about" class="btn btn-secondary">Edit About / Bio</a>
                <a href="/admin/settings#contact" class="btn btn-secondary">Edit Contact Info</a>
                <a href="/admin/awards" class="btn btn-secondary">Manage Awards</a>
            </div>
        </div>
    """
...
@app.route("/admin/awards", methods=["GET"])
@login_required
def admin_awards_page():
    settings = _get_settings()
    awards = settings.get("awards", [])
    return render_template("admin/awards.html", active_page="awards", awards=awards)
...
def api_admin_create_project():
    try:
        ...
        logger.exception("api_admin_create_project failed")
...
def api_admin_update_project(slug):
    try:
        ...
        logger.exception("api_admin_update_project failed")
        return jsonify({"error": str(e)}), 500
...
def api_admin_delete_project(slug):
    try:
        ...
        logger.exception("api_admin_delete_project failed")
        return jsonify({"error": str(e)}), 500
...
def api_admin_put_skills():
    try:
        ...
        logger.exception("api_admin_put_skills failed")
        return jsonify({"error": str(e)}), 500
...
def api_admin_post_experience():
    try:
        ...
        logger.exception("api_admin_post_experience failed")
        return jsonify({"error": str(e)}), 500
...
def api_admin_put_experience(index):
    try:
        ...
        logger.exception("api_admin_put_experience failed")
        return jsonify({"error": str(e)}), 500
...
def api_admin_delete_experience(index):
    try:
        ...
        logger.exception("api_admin_delete_experience failed")
        return jsonify({"error": str(e)}), 500
...
def api_admin_create_software():
    try:
        ...
        logger.exception("api_admin_create_software failed")
        return jsonify({"error": str(e)}), 500
...
def api_admin_update_software(id):
    try:
        ...
        logger.exception("api_admin_update_software failed")
        return jsonify({"error": str(e)}), 500
...
def api_admin_delete_software(id):
    try:
        ...
        logger.exception("api_admin_delete_software failed")
        return jsonify({"error": str(e)}), 500
...
def api_admin_toggle_software_status(id):
    try:
        ...
        logger.exception("api_admin_toggle_software_status failed")
        return jsonify({"error": str(e)}), 500
@app.route("/api/admin/settings/awards", methods=["GET"])
@login_required
def api_admin_get_awards():
    settings = _get_settings()
    return jsonify(settings.get("awards", []))

@app.route("/api/admin/settings/awards", methods=["POST"])
@login_required
@csrf_required
def api_admin_post_award():
    try:
        data = request.get_json(force=True)
        settings = _get_settings()
        if "awards" not in settings:
            settings["awards"] = []
        settings["awards"].append(data)
        settings["updatedAt"] = now_iso()
        _save_settings(settings)
        return jsonify(data)

    except Exception as e:
        logger.exception("api_admin_post_award failed")
        return jsonify({"error": str(e)}), 500
@app.route("/api/admin/settings/awards/<int:index>", methods=["PUT"])
@login_required
@csrf_required
def api_admin_put_award(index):
    try:
        data = request.get_json(force=True)
        settings = _get_settings()
        awards = settings.get("awards", [])
        ...
        logger.exception("api_admin_put_award failed")
        return jsonify({"error": str(e)}), 500
@app.route("/api/admin/settings/awards/<int:index>", methods=["DELETE"])
@login_required
@csrf_required
def api_admin_delete_award(index):
    try:
        settings = _get_settings()
        awards = settings.get("awards", [])
        if 0 <= index < len(awards):
        ...
        logger.exception("api_admin_delete_award failed")
        return jsonify({"error": str(e)}), 500
...
def api_admin_put_settings():
    try:
        ...
        # Normalize tools: ensure it's an array
        if "tools" in settings and isinstance(settings["tools"], str):
            settings["tools"] = [t.strip() for t in settings["tools"].split(",") if t.strip()]
        # Normalize sectionLabels: ensure it's a dict if it looks like JSON
        if "sectionLabels" in settings and isinstance(settings["sectionLabels"], str):
            try:
                settings["sectionLabels"] = json.loads(settings["sectionLabels"])
            except Exception:
        ...
        logger.exception("api_admin_put_settings failed")
        return jsonify({"error": str(e)}), 500
...
def api_admin_profile_image():
    ...
    _save_media(media)
    settings = _get_settings()
    settings["profileImage"] = media_item["url"]
    settings["updatedAt"] = now_iso()
    _save_settings(settings)
    return jsonify({"media": media_item, "settings": settings})

@app.route("/api/admin/profile-image/clear", methods=["POST"])
@login_required
@csrf_required
def api_admin_profile_image_clear():
    settings = _get_settings()
...
def admin_seo_put():
    try:
        ...
        logger.exception("admin_seo_put failed")
        return jsonify({"error": str(e)}), 500
